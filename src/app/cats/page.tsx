...
export default function CatsPage() {
    const [filters, setFilters] = useState<Filters>({ q: '', type: 'CAT', size: 'ALL', sex: 'ALL', age: 'ALL' });
    ...
    return (
        <section id="listado" className="space-y-4 py-8 px-24">
            <div className="flex items-center justify-between">
                <h2 className="text-2xl font-bold" style={{ color: CASI_NEGRO }}>Gatos en adopción</h2>
                <span className="text-sm" style={{ color: CASI_NEGRO + '99' }}>{total} resultados</span>
            </div>

            <FilterBar
                onChange={(f) => {
                    // Mantener el tipo como CAT siempre
                    setFilters({ ...f, type: 'CAT' });
                }}
                hideTypeFilter={true}
            />
            ...
            {isLoading ? (
                ...
                <div className="py-16 text-center text-red-600" style={{ color: CASI_NEGRO }}>Ocurrió un error cargando los gatos.</div>
            ) : (
                <>
